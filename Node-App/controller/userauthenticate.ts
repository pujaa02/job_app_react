...
const register = async (req: Request, res: Response) => {
  ...
  try {
    const registerinsert: Model<UserAttributes, UserCreationAttributes> =
      await User.create({
        fname: fname,
        lname: lname,
        email: email,
...
const activatecheck = async (req: Request, res: Response) => {
  ...
  const result: UserAttri | null = await User.findOne({
...
const deleteuser = async (req: Request, res: Response) => {
  ...
  await User.update(
    { isdeleted: 1, deleted_at: new Date() },
    { where: { user_id: user_id } }
  );
...
const checkuser = async (req: Request, res: Response) => {
  ...
  try {
    ...
    if (result?.dataValues) {
      const isPassSame: boolean = await bcrypt.compare(
        pass,
      ...
      if (isPassSame === true) {
        ...
        res
          ...
          .json({ msg: "Success", token, user_id: result?.dataValues.user_id });
      } else {
